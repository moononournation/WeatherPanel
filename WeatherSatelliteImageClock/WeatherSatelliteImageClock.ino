...
/* select below one and only one of satellite image source */
  ...
  while (minute_i < rounding_time)
  {
    ...
    if (SD_MMC.exists(path))
    {
      if (jpeg.open(path, jpegOpenSD_MMC, jpegClose, jpegRead, jpegSeek, JPEGDraw))
      {
        ...
        jpeg.setCropArea(SATELLITE_IMAGE_X, SATELLITE_IMAGE_Y, gfx->width(), gfx->height()); // requested area
